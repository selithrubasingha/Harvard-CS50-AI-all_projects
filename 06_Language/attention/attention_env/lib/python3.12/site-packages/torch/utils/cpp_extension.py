...
def CUDAExtension(name, sources, *args, **kwargs):
    ...
    Using `-dlto` (Device Link Time 03_Optimization) at the device code compilation step and `dlink` step
    ...
    if dlink:
        ...
        if (torch.version.cuda is not None) and TorchVersion(torch.version.cuda) >= '11.2':
            extra_compile_args_dlink += ['-dlto']   # Device Link Time 03_Optimization started from cuda 11.2
